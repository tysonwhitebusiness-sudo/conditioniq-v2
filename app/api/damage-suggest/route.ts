...
//
//   POST { inspectionId, slot } → { ok, found } or { ok: false, reason }
//
...
export async function POST(request: Request) {
  ...
  try {
    return NextResponse.json(await suggestDamageForSlot(inspectionId, auth.companyId, slot as ExteriorSlot))
  } catch (err) {
