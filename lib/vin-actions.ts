...
export async function decodeVINAction(vin: string): Promise<VINDecodeResult | null> {
  if (!vin || vin.length !== 17) return null
  try {
    const res = await fetch(
      `https://vpic.nhtsa.dot.gov/api/vehicles/decodevin/${vin}?format=json`,
      { next: { revalidate: 86400 } } // cache for 24h server-side
    )
    if (!res.ok) return null
    const json = await res.json()
    const vars: { Variable: string; Value: string | null }[] = json.Results ?? []
