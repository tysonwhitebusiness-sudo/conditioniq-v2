...
## 2026-07-09 — Production build failure: inline `'use server'` in a file also exporting plain client-callable functions (not a new vulnerability — a deployment-breaking regression from Follow-up Step 3)

**Finding addressed, and severity:** Build-breaking, confirmed live in production — Vercel's build log showed `Failed to compile: It is not allowed to define inline "use server" annotated Server Actions in Client Components`, pointing at `lib/storage-actions.ts:50`, inside `upsertVehicleToInventory`. Not a security vulnerability in itself, but it meant **no deployment could succeed at all**, including every fix in this remediation sequence — worth logging here given the severity and its direct connection to a prior security-motivated code change.

**Root cause:** `upsertVehicleToInventory` was given a function-local (not file-level) `'use server'` directive in Follow-up Step 3, specifically to avoid converting the whole `lib/storage-actions.ts` file to a server-only module, since that file's other exports (`getStorageVehicles`, `addStorageVehicle`, etc.) are intentionally client-callable against the session-bound browser client. That reasoning was sound, but the implementation choice — one inline `'use server'` function coexisting with plain client-facing exports in the same file — is a pattern Next.js's compiler disallows once the file is pulled into a Client Component's module graph, which it is: `components/inspection-wizard/inspection-wizard.tsx` (`'use client'`) dynamically imports `upsertVehicleToInventory` from this exact file. This was never caught at the time, because Follow-up Step 3's only verification was `tsc --noEmit` — which checks TypeScript types, not Next.js's Server Actions bundling rules, and passed clean despite the underlying structural problem. `npm run build` was not run after that fix; this class of error only surfaces there.

**What changed:**
- New file, `lib/storage-server-actions.ts`, with a proper file-level `'use server'` directive — following Next.js's own suggested fix ("export them from a separate file with 'use server' at the top") and matching this codebase's existing convention of paired client/server files (`lot-actions.ts`/`lot-server-actions.ts`). Contains only `upsertVehicleToInventory`, moved verbatim (identical logic, identical behavior) — the two previously-dynamic imports inside the function (`authorizeInspectionAccess`'s sibling `authorizeCompanyAccess`, and `createAdminClient`) are now static top-of-file imports, since the original reason for dynamic-importing them (avoiding pulling admin/service-role code into a client bundle via this file's *other* exports) no longer applies — this file has no other exports, and is a proper server-only module.
- `lib/storage-actions.ts`: `upsertVehicleToInventory` removed entirely (lines 41-134, including its section comment) — every other export in the file is untouched.
- `components/inspection-wizard/inspection-wizard.tsx`: its dynamic import (`await import('@/lib/storage-actions')`) now points to `@/lib/storage-server-actions` instead — a one-line change, no other logic touched.
- `tsc --noEmit` — clean, exit code 0.
- **`npm run build` — run for the first time against this specific code path since Follow-up Step 3 shipped it, and confirmed the actual fix**, not just a type-check: first attempt hit an unrelated Windows-local filesystem error (`UNKNOWN: unknown error, open '...\(app)\customers\[customerId]\page.js'`) from a stale `.next` build cache; cleared it (`rm -rf .next`) and reran — full production build succeeded, all 43 routes compiled, zero errors, only pre-existing unrelated Sentry/Edge-Runtime deprecation warnings.

**Why this closes the issue:** the file containing the inline `'use server'` function no longer has any plain client-facing exports coexisting with it — it's now a dedicated, unambiguous server-actions module, which is exactly the structure Next.js requires. `lib/storage-actions.ts` goes back to being a pure client-callable module with no server action inside it at all.

**Legitimate flows confirmed still working:** identical function body, identical logic, identical authorization check (`authorizeCompanyAccess`), identical admin-client usage — only the file location and import style changed. The sole caller (`inspection-wizard.tsx`'s `handleComplete`) was updated to the new import path; no other call site exists (confirmed via repo-wide search before and after this fix).

**Deploy ordering:** code-only change, no migration involved, no ordering constraint — this is what needs to ship to unblock deployment entirely.

**Open follow-ups / caveats:**
- **Status: fix applied, `tsc --noEmit` clean, and `npm run build` independently confirmed successful in this session — this is a stronger verification than most entries in this log, which were typically `tsc`-only with runtime testing deferred.** Still recommend confirming the next real Vercel deployment succeeds, since a local Windows build and Vercel's Linux build environment are not identical, though the specific error this fix targets is a Next.js compiler-level rule, not an OS-specific behavior.
- This is a direct, concrete illustration of `SECURITY_CHECKLIST.md` item 10 ("testing is not optional just because the code review was thorough") — Follow-up Step 3's fix was well-reasoned and passed the verification performed at the time, but that verification didn't cover the failure mode that actually shipped.
