...
const CONFIGS: Record<string, { label: string; bg: string; text: string }> = {
  queued:           { label: 'Queued',          bg: '#F3F4F6', text: '#374151' },
...
export default function StatusBadge({ status, className = '' }: StatusBadgeProps) {
  const config = CONFIGS[status] ?? { label: status, bg: '#F3F4F6', text: '#374151' }
  return (
    <span
...
export function ScoreBadge({ score }: { score: number }) {
  ...
  return (
    ...
    >
      ...
      <span className="text-[8px] font-semibold uppercase tracking-wider mt-0.5" style={{ color: '#6B7280' }}>score</span>
    </div>
  )
}
